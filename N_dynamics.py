...
def growth_rates(N,param,mat,up):
    ...
    for i in range(n_s):
        ...
        growth_matrix += species_i_matrix*param['g'][i]*(np.sum(uptake*(1-l)-param['m'][i],axis=2))
...
def death_birth_periodic(state,G):
    ...
    return state, 'ancora', np.argmax(np.bincount(state.ravel()))
